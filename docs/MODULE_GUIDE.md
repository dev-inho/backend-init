...
### 9. storage:mybatis
> **공유 도메인 엔티티**
> `gradle/domain-entities.gradle`에 선언된 공통 도메인 엔티티 정의(Sample, User, FileMeta)를 `storage/jpa`와 `storage/mybatis` 양쪽에서 공유하여 사용합니다. JPA 테스트 및 검증용 픽스처(ScalarSample, RelationParent, RelationChild)는 **storage/jpa 전용**으로 격리되어 관리되며 공유 스크립트에 포함되지 않습니다.
