...
2. `support:jwt`, `support:logging`, `core:domain` (`support:util` 의존 / 도메인은 의존성 0)
3. `support:web`, `client:storage-file`, `storage:mybatis`, `storage:jpa`, `gateway:core` (`support:*`, `core:domain` 의존)
4. `gateway:autoconfigure` (`gateway:core` 의존)
5. `gateway:starter` (`gateway:core`, `gateway:autoconfigure` 의존)
6. `core:batch`, `gateway:app`, `core:application` (도메인, 어댑터, 스타터/지원 모듈 의존)
