...
* **직접 구현체 Import (테스트) -> [복원됨 (PR #43)]:**
  * 과거 감사 대상 파일 (5개):
    ...
    * 프로바이더 컨텍스트 테스트 (2개, provider 프로퍼티 기반 자동 구성 검증으로 복원):
      * `core/application/src/test/kotlin/cc/midolog/persistence/JpaProviderContextTest.kt`
      * `core/application/src/test/kotlin/cc/midolog/persistence/MyBatisProviderContextTest.kt`
  * **복원 근거 및 현재 상태:**
    ...
    - 저장소 어댑터 등록 방식은 기존 Spring Profile 및 호스트 컴포넌트 스캔 의존 방식에서 Spring Boot 표준 Starter 자동 구성(`AutoConfiguration.imports`, `@ConditionalOnProperty`, `@ConditionalOnMissingBean`)으로 전면 복원되었습니다 (PR #49).
    - 컨텍스트 테스트 2개(`JpaProviderContextTest.kt`, `MyBatisProviderContextTest.kt`)는 concrete storage 패키지 import 및 FQCN 직접 참조 없이, 순수 도메인 포트 인터페이스(`*RepositoryPort`)와 `storage.persistence.provider=jpa|mybatis` 프로퍼티 기반으로 단일 어댑터 빈 등록 및 쿼리 동작을 검증합니다.
    - 현재 `core/application` 및 `examples/minimal-app` 내에는 concrete storage 어댑터 import가 0건이며, `PersistenceBoundaryTest` 가드가 영속성 관심사 재유출을 원천 차단합니다.
