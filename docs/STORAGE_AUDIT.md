...
- **예외 변환 한계:** JPA 어댑터는 트랜잭션 콜백 null 발생 시 하드코딩된 예외를 던지며, MyBatis는 `affectedRows == 1`을 검사하여 예외를 발생시키나, 이를 동시성 제어라고 과장해서는 안 되며 단순 기대 행 수 검증에 불과합니다.
...
* **판정:** 결함
* **근거:** `ClockConfig` 주입(`clock.instant()`) 대신 `Instant.now()` 하드코딩을 사용하여 테스트 시간 의존성 문제가 있습니다.
...
* **판정:** 개선
* **근거:** 엔티티 필드는 nullable 여부에 맞게 잘 생성되나 Mapper가 수동 매핑하여 스키마 변경 시 누락 위험이 있습니다. 관계 미존재로 현재 N+1 이슈는 없으나 추후 연관관계 설정 시 자동 생성이 필요합니다.

## 5. 포트 추가/이중화 유지보수성 평가
PR #24 기준, 포트 1건 추가 시 20개 파일이 변경되며 이 중 7개(`MyBatisFileMetaRepositoryAdapterTest.kt` 등 JPA 2개, MyBatis 5개)가 순수 이중화를 위한 유지 비용입니다.

### 5.1 이중화 유지 및 통폐합 후보 추천 비교안
* **유지:** 데이터 접근 선택지는 넓으나 구현 및 테스트 비용(1건당 7파일 추가)이 너무 큽니다.
* **JPA 단일 (추천):** 객체지향적 도메인 매핑과 타입 안전성(JDSL 등) 확보가 용이하며 컴파일 타임 검증을 통해 MyBatis 관련 비용을 제거합니다.
* **MyBatis 단일:** XML 및 Map 기반 매핑 의존과 SQL 수동 유지보수로 인해 생산성이 떨어집니다.
...
* **프레임워크 설정 문자열 유출:**
  ...
  * `core/application/src/main/resources/application.yml:4` - `spring:` / `data: jpa: repositories: enabled: false` 등 (실제 yml 4-16행, 39-46행 JPA/MyBatis 설정 키 유출)
* **모듈 의존성 유출:**
